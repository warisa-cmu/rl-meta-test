...
INJECTED_SOLUTION_LARGE_3 = np.array(
    [
        0.25860524,
        0.00974619,
        0.17761059,
...
class VRPTW:
    ...
    def reset(self, seed=42):
        ...
        # Inject known good solution for LARGE3 problem set
        if self.dimensions == INJECTED_SOLUTION_LARGE_3.shape[0]:
            if self.verbose > 0:
                print("Injecting known good solution into population[0]")
            self.population[0 : int(self.population_size * 0.2)] = (
                INJECTED_SOLUTION_LARGE_3
            )
        else:
            raise ValueError("Dimension mismatch when injecting known good solution.")
