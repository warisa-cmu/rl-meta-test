...
from P02_MSIE.T06_exp.vrptw_v9 import (
    CR_LOWER_BOUND,
    CR_UPPER_BOUND,
    F_LOWER_BOUND,
    F_UPPER_BOUND,
...
class AIMH_ENV(gym.Env):
    def __init__(self, vrp):
        ...
        self.action_space = spaces.Box(
            low=np.array(
                [F_LOWER_BOUND, CR_LOWER_BOUND, MG_LOWER_BOUND], dtype=np.float32
            ),
            high=np.array(
                [F_UPPER_BOUND, CR_UPPER_BOUND, MG_UPPER_BOUND], dtype=np.float32
            ),
            shape=(3,),
            dtype=np.float32,
        )
