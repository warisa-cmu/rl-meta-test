...
class VRPTW:
    ...
    def __init__(
        ...
        self.F_bounds = [F_LOWER_BOUND, F_UPPER_BOUND]
        self.CR_bounds = [CR_LOWER_BOUND, CR_UPPER_BOUND]
    ...
    def reset(self, seed=42):
        ...
        self.MG_rate = 0.5
        self.F_rate = (self.F_bounds[0] + self.F_bounds[1]) / 2
        self.CR_rate = (self.CR_bounds[0] + self.CR_bounds[1]) / 2
    ...
    # --------------------------
    def check_FCR(self):
        if self.F_rate < self.F_bounds[0]:
            self.F_rate = self.F_bounds[0]

        if self.CR_rate < self.CR_bounds[0]:
            self.CR_rate = self.CR_bounds[0]

        if self.F_rate > self.F_bounds[1]:
            self.F_rate = self.F_bounds[1]

        if self.CR_rate > self.CR_bounds[1]:
            self.CR_rate = self.CR_bounds[1]
    ...
    def calc_std_population(self):
        ...
        if np.all(self.current_cost == SOL_UPPER_BOUND):
